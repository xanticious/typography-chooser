  ...
  return (
    <div className={`${styles.demo} ${styles.dashboardLayout}`}>
      ...
      <div className={styles.dashboardContent}>
        ...
        <p className={styles.body}>Returns a paginated list of users belonging to the authenticated account.</p>
        <pre className={styles.codeBlock}><code>GET /v1/users?page=1&limit=25</code></pre>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            ...
            <tr><td>limit</td><td>integer</td><td>No</td><td>Results per page (max 100)</td></tr>
            <tr><td>role</td><td>string</td><td>No</td><td>Filter by role: admin, member</td></tr>
          </tbody>
        </table>
        <h2 className={styles.subheading}>Example Response</h2>
        <pre className={styles.codeBlock}><code>{`{"data": [{"id": "u_01", "email": "alice@example.com", "role": "admin"}], "total": 42}`}</code></pre>
      </div>
    </div>
  );
}
